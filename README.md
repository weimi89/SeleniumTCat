...
reports/               # 執行報告 (JSON 格式)
├── 20260109_124852.json    # 含功能名稱、帳號統計、執行時間
└── ...

logs/                 # 執行日誌
temp/                 # 暫存檔案 (UUID 臨時目錄，完成後自動清理)
```
...
設定 `DISCORD_WEBHOOK_URL` 後，系統會在所有帳號處理完成時自動發送通知：
...
### 🔧 ChromeDriver 自動匹配 (2026-01)
- **版本自動匹配**: 優先使用 WebDriver Manager 自動下載匹配版本
- **後備機制**: 系統 ChromeDriver 作為後備方案
- **無需手動更新**: Chrome 瀏覽器更新後自動適應
...
A: 程式會自動使用 WebDriver Manager 下載匹配版本，通常無需手動處理
