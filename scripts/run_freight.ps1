...
try {
    ...
    if ($args.Count -eq 0 -or (-not ($args -contains "--start-date") -and -not ($args -contains "--end-date"))) {
        ...
        # 取得上個月第一天
        $today = Get-Date
        $firstDayOfThisMonth = $today.AddDays(-($today.Day - 1))
        $lastMonth = $firstDayOfThisMonth.AddDays(-1)
        $startDate = $lastMonth.AddDays(-($lastMonth.Day - 1)).ToString("yyyyMMdd")
        # 計算上個月的最後一天
        $endDate = $lastMonth.ToString("yyyyMMdd")
