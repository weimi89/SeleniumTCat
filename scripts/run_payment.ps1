# 黑貓宅急便客樂得對帳單自動下載工具 - PowerShell 7 版本
...
Write-Host "📥 黑貓宅急便客樂得對帳單自動下載工具" -ForegroundColor Cyan
...
try {
    ...
    if (-not ($args -contains "--period")) {
        ...
        Write-Host "• 2 = 下載最新 2 期（預設）"
        Write-Host "• 3 = 下載最新 3 期"
        Write-Host "• 0 或空白 = 下載最新 2 期"
        Write-Host ""
        ...
        if ($periodInput -and $periodInput -match '^\d+$' -and [int]$periodInput -gt 0) {
            ...
            $args += "--period"
            $args += "2"
            Write-Host "✅ 使用預設值：下載最新 2 期" -ForegroundColor Green
        }
        Write-Host ""
    }
    ...
    # 執行 Python 程式
    Write-Host "🚀 啟動客樂得對帳單查詢功能" -ForegroundColor Blue
    Write-Host ""
